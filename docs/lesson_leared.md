...
- **반복 방지 체크**: 지연 응답·인증 전환·저장 중 복구 회귀와 실제 API/DB 브라우저의 저장 후 새로고침·두 층 왕복을 함께 유지한다. 반납 요청이 전달되지 않는 브라우저 종료/네트워크 손실에서는 서버 만료와 명시적 재시도를 안전 경계로 남긴다.
...
## 2026-09-10 / 격자 스냅은 절대 좌표와 저장 계약을 함께 가져야 함
- **발생했던 문제/실수**: 격자 스냅이 조명 드래그의 이동량만 10 단위로 반올림해 시작 좌표가 격자 밖이면 결과도 계속 어긋났고, 도형 생성·크기 변경·키보드 이동에는 적용되지 않았다. 배경 없는 층은 격자 간격을 저장할 데이터 행도 없었다.
- **원인**: 스냅을 캔버스 표시 옵션으로만 보고 맵의 영속 설정과 전체 편집 동작의 공통 좌표 규칙으로 정의하지 않았다.
- **해결 및 예방책**: 층별 `FloorPlan.gridSize`를 DB·공유 스키마·API·revision snapshot에 포함하고, 모든 편집 경로는 맵 절대 좌표의 시작점과 끝점을 같은 helper로 스냅한다. 배경 없는 맵도 `sourceType = none`으로 크기와 격자를 저장한다.
- **반복 방지 체크**: 좌표 보조 기능은 생성·드롭·이동·리사이즈·키보드·저장/복구·읽기 모델을 모두 테스트하고, delta가 아니라 최종 절대 좌표가 격자 배수인지 검증한다.

## 2026-09-10 / 캔버스 이동 미리보기와 좌표 확정은 분리해야 함
- **발생했던 문제/실수**: 격자 스냅을 드래그 중 매 프레임 적용해 요소가 포인터를 부드럽게 따르지 않았고, 도형의 위치만 옮겨도 격자 정규화가 기존 너비와 높이까지 바꿀 수 있었다.
...
- **발생했던 문제/실수**: shared build manifest 경로를 `resolve()`로 `dist` 아래인지 확인했지만 `dist/esm`이 외부 디렉터리 symlink로 바뀌면 cleanup `rm()`이 외부의 같은 이름 파일을 삭제했다. Symlink와 drive/ADS를 보강한 뒤에도 POSIX에서 Windows reserved device basename을 일반 파일로 수락했다.
