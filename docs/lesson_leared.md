...
- **발생했던 문제/실수**: shared build manifest 경로를 `resolve()`로 `dist` 아래인지 확인했지만 `dist/esm`이 외부 디렉터리 symlink로 바뀌면 cleanup `rm()`이 외부의 같은 이름 파일을 삭제했다. Symlink 보강 뒤에도 POSIX에서 Windows drive-relative와 ADS path를 literal filename으로 수락했다.
- **원인**: 문자열 경로의 `..` escape만 차단하고 `dist` root부터 target parent까지 실제 component type을 확인하지 않았다. Backslash를 slash로 정규화한 뒤 검사해 `C:..\\outside.js`와 `index.js:stream`이 Windows에서 갖는 의미도 잃었다.
- **해결 및 예방책**: manifest 전체와 생성 대상 전체를 mutation 전에 POSIX logical path validator와 `lstat` chain으로 선검증하고, cleanup/generation/manifest read·write 및 unlink/rmdir/mkdir/write 직전에 다시 검사한다. Colon/backslash와 terminal dot/space는 정규화 없이 거부한다. Target symlink와 directory/non-file은 fail-closed하며 parent는 real directory만 한 단계씩 생성한다. Generated file은 exclusive temporary regular file로 쓴 뒤 `rename`해 final symlink를 따라가지 않는다.
- **반복 방지 체크**: build cleanup 테스트에는 `dist` root symlink, intermediate parent symlink, target file symlink, upper/lower drive absolute·relative, ADS, UNC/device, backslash/mixed separator, terminal dot/space, traversal/empty/duplicate/directory/NUL manifest와 기존 artifact·외부 sentinel을 포함한다. Node에 dirfd-relative `openat`/`unlinkat`이 없어 남는 parent-swap race는 writable build directory 권한과 단일 build process로 추가 제한한다.
