...
- **해결 및 예방책**: API publisher는 30초 lease보다 짧은 10초 timeout으로 timeout/reject를 backoff/dead-letter terminal로 전환한다. Gateway는 logical scan key와 terminal payload를 0600 atomic journal에 보존해 running duplicate를 차단한다. restart는 running을 publish 전 정제 failed terminal로 원자 전환하고, command topic 구독 뒤 connect-ready에서 미전달 terminal만 same eventId/sequence로 직렬 재시도하며, 성공은 delivered marker로 기록한다. corrupt journal은 새 scan을 실행하지 않고 fail-closed 한다.
- **반복 방지 체크**: migration rehearsal에는 historical session duplicate를 migration 전에 넣고, publisher에는 stalled callback/lease 경계 테스트를, Gateway에는 duplicate scan-start, restart running recovery, failed terminal reconnect replay, concurrent drain, corrupt journal 테스트를 유지한다.
