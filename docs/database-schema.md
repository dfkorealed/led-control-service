...
자동화 membership 변경은 낮은 빈도의 구성 쓰기라는 전제에서 의도적으로 transaction 단위 직렬화한다. `LightingScheduleFixture`, `VehicleEventSource`, `VehicleEventTarget`, `ManualOverrideFixture`의 INSERT/UPDATE/DELETE `BEFORE STATEMENT` trigger는 top-level DML의 target tuple이 잠기기 전에 동일한 `pg_advisory_xact_lock(1279607873, 1296387394)`를 획득한다. 고정 key의 mnemonic은 `LEDA`/`MEMB`이며 transaction 전체에 유지하므로 여러 parent를 반대 순서로 바꾸는 multi-row statement도 직렬화된다. Parent DELETE가 만든 nested FK cascade는 `pg_trigger_depth() > 1`이고 이미 parent row/FK cascade가 직렬화 권한을 가지므로 advisory lock을 다시 얻지 않는다. 이 cascade의 row maintenance는 삭제 중인 parent가 이미 보이지 않을 때 counter 갱신을 생략하고, depth 1에서 나중에 실행되는 deferred event도 parent 부재를 확인해 cardinality reconciliation을 생략한다. 반면 direct top-level child DELETE는 depth 1 statement lock, counter 감소, deferred 최소 1개 검증을 모두 수행한다.
...
- Schedule target, vehicle source/target, manual target의 top-level DML은 모두 `BEFORE STATEMENT`에서 고정 key `(1279607873, 1296387394)`의 transaction-level advisory lock을 target tuple보다 먼저 획득하고 parent-maintained non-negative counter를 갱신한다. 이 낮은 빈도의 구성 쓰기 직렬화가 multi-row 반대 parent 변경의 transaction-global lock cycle을 제거한다. Parent-key move의 양쪽 parent ID 정렬, DEFERRABLE INITIALLY DEFERRED counter/실제 child 수 reconciliation, 최소 1개 제약은 그대로 유지한다. Parent DELETE의 nested FK cascade는 trigger depth와 삭제된 parent 부재를 함께 확인해 advisory/counter/deferred 작업을 건너뛰고 parent row/FK cascade 순서를 따른다.
