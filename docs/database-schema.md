...
다중 API 인스턴스에서는 `lockedBy`, `lockedAt`, `leaseExpiresAt`으로 30초 발행 lease를 소유하고 PostgreSQL `FOR UPDATE SKIP LOCKED`로 같은 레코드의 중복 발행을 차단한다. Command payload는 생성 직후 strict draft, fix round 4 이후 publisher가 확정한 strict published generation, 또는 rolling upgrade 중 남은 legacy full wire일 수 있다. Compatibility parser는 legacy near-expiry wire를 복구용으로만 읽고 임의 추가 키는 허용하지 않는다. 새 producer는 `expiresAt <= overrideUntil`과 generation metadata 상호 일치를 모두 만족하는 strict published wire만 만든다.
...
순방향 migration `20260904_bind_manual_execution_command_source`는 manual `action_result`의 wire identity와 DB relation identity를 구분한다. Gateway payload의 `sourceId=Command.id`를 `ManualOverride.commandId`로 조회하고, 실행 row의 `manualOverrideId`가 그 override의 실제 PK일 때만 허용한다. 따라서 command ID와 override PK가 다른 정상 실행은 저장되며, 다른 override PK를 결속한 실행은 `23514`로 거부된다.
