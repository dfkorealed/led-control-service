...
자동화 membership 변경은 낮은 빈도의 구성 쓰기라는 전제에서 의도적으로 transaction 단위 직렬화한다. `LightingScheduleFixture`, `VehicleEventSource`, `VehicleEventTarget`, `ManualOverrideFixture`의 모든 INSERT/UPDATE/DELETE maintenance trigger는 parent counter나 child row가 바뀌기 전에 동일한 `pg_advisory_xact_lock(1279607873, 1296387394)`를 획득한다. 고정 key의 mnemonic은 `LEDA`/`MEMB`이며, transaction 전체에 유지해 여러 parent를 반대 순서로 변경해도 row-lock cycle이 생기지 않게 한다. 같은 transaction의 후속 trigger 호출과 parent cascade delete는 같은 lock을 재사용한다.
...
`LightingScheduleFixture_target_count_maintenance`는 공통 automation membership advisory lock을 먼저 획득한다. INSERT/DELETE에서 부모 `targetCount`를 원자 증감하고 `scheduleId` UPDATE에서는 OLD/NEW 부모를 ID 오름차순 `FOR UPDATE`로 잠근 뒤 두 counter를 갱신한다. 음수는 DB check와 underflow guard가 거부한다. DEFERRABLE INITIALLY DEFERRED constraint trigger는 commit 시 `targetCount >= 1`과 실제 child `COUNT(*)` 일치를 함께 검증한다. Parent+child를 같은 transaction에서 만들거나 snapshot 전체를 교체할 수 있고, 이미 삭제된 parent는 건너뛰어 parent cascade를 허용한다. 모든 child write가 같은 parent row version을 갱신하므로 READ COMMITTED는 lock 대기 뒤 최신 counter로 재검사하고 REPEATABLE READ/SERIALIZABLE은 concurrent row update를 serialization failure로 종료해 동시 마지막-target 삭제의 stale 성공을 막는다.
...
각 child maintenance trigger는 공통 automation membership advisory lock을 먼저 획득한 뒤 INSERT/DELETE/`ruleId` UPDATE에 맞춰 `sourceCount` 또는 `targetCount`를 원자 갱신한다. 부모 이동은 두 rule row를 ID 오름차순으로 잠근다. Deferred 검증은 두 counter가 각각 1 이상이고 실제 source/target row 수와 정확히 같은지 확인하며, parent cascade 삭제는 허용한다. 같은 parent row version을 쓰는 방식이 READ COMMITTED, REPEATABLE READ, SERIALIZABLE의 동시 마지막-child 삭제를 직렬화하거나 serialization failure로 종료한다.
...
Child maintenance trigger는 공통 automation membership advisory lock을 먼저 획득하고 INSERT/DELETE/부모-key UPDATE에서 `targetCount`를 원자 갱신하며 부모 이동 시 두 override row를 ID 오름차순으로 잠근다. Deferred 검증은 `targetCount >= 1`과 실제 target row 수 일치를 강제하며 parent cascade 삭제는 허용한다. 동일 parent row version 갱신이 모든 공통 isolation level에서 동시 마지막-target 삭제의 stale 성공을 막는다. Command/User/Fixture 관계를 restrict해 이미 실행된 수동 override 원장이 참조 대상 삭제로 유실되지 않게 한다.
...
- Schedule target, vehicle source/target, manual target은 모두 고정 key `(1279607873, 1296387394)`의 transaction-level advisory lock을 parent counter와 membership 변경 전에 획득하고 parent-maintained non-negative counter를 갱신한다. 이 낮은 빈도의 구성 쓰기 직렬화가 multi-row 반대 parent 변경의 transaction-global lock cycle을 제거한다. Parent-key move의 양쪽 parent ID 정렬, DEFERRABLE INITIALLY DEFERRED counter/실제 child 수 reconciliation, 최소 1개 제약은 그대로 유지하며 parent cascade로 이미 삭제된 row는 건너뛴다.
