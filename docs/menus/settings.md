...
- `/settings/floor-plans/:floorId/edit`는 route param으로 `GET /floors/:floorId/editor-state`를 조회하고 기존 `FloorEditorView`의 저장·취소 callback으로 도면 목록으로 이동한다. `viewer`는 목록과 도면 등록 상태만 읽을 수 있으며, 직접 edit URL도 목록으로 redirect되어 editor state를 조회하거나 편집기를 렌더링하지 않는다.
...
- Task 8 atomic save/revision API는 `d8d42f1`, Fix Round 1은 `edde0a8`/`8d7aa2e`, Fix Round 2는 `a3aa1b7`/`669be7e`/`d8041f6`, Fix Round 3의 effective legacy plan과 opaque restore access 보정은 `63cd6fd`/`45336dc`에서 완료했다. 다음 구현 범위는 Task 9의 웹 변경분 생성, atomic save 연결, 충돌·복구 UI다.
