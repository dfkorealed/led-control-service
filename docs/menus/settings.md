...
- 도면 편집 route는 진입 시 lease를 얻고 editable token이면 30초마다 single-flight heartbeat로 갱신한다. heartbeat가 상실되면 같은 effect에서 다시 editable로 바뀌지 않으며, 이전 floor 요청의 늦은 성공·실패도 현재 floor lease를 건드리지 못한다. 처음 read-only 충돌은 250ms부터 최대 15.75초의 bounded retry로 delayed cleanup release를 회복하지만, `SET NX`만 사용하므로 다른 사용자의 lease를 빼앗지 않는다. 충돌, 갱신 실패, token 상실, lease 획득 실패와 floor 전환 중에는 저장/복구/도구/캔버스/배경/속성 변경을 막는 읽기 전용으로 전환한다. 정상 route 이탈은 token release를 요청하고, 브라우저 종료 같은 비정상 종료의 회수는 90초 TTL에 맡긴다. 이 lease는 revision `409` optimistic concurrency를 대체하거나 강제 덮어쓰기를 허용하지 않는다.
