...
- Gateway는 automation state schema v3에 실제 관측 또는 성공 terminal로 확인된 desired와 restart observation이 필요한 legacy desired를 분리한다. V1 및 terminal 증거 없는 V2 desired와 V2/V3 pending은 lighting observation까지 RF를 보류하고, target과 관측 brightness가 같으면 terminal로 확정하며 다를 때만 RF를 전송한다. RF success 뒤 terminal state write failure/commit uncertainty도 same-process fence를 즉시 세우고 해당 fixture를 bounded targeted lighting-resync queue에 요청한다. 요청 실패나 미관측은 지수 backoff로 재시도하고 unresolved batch를 insertion-order tail로 회전시켜 선두 64개가 offline이어도 이후 fixture가 관측 기회를 얻는다. 요청이 4,096개 capacity를 넘으면 수용 가능한 fixture는 64개 batch queue에 유지하고 나머지는 active full resync 뒤의 guaranteed rerun으로 승격한다. Durable OnOff/Lightness 관측 반영 뒤에만 fence와 queue 항목을 해제한다.
- Automation fixture observation은 Health Current telemetry와 분리된 OnOff/Lightness callback으로 수집한다. Health timeout이어도 밝기 recovery는 완료된다. Full resync는 MQTT/heartbeat/ACK/manual control startup 뒤 모든 confirmed fixture를 bounded-concurrency worker로 순회하고 fixture 실패를 격리한다. Full/targeted worker는 fixture 경계, 관측 대기, busy retry에 `AbortSignal`을 전달하고 production shutdown은 기본 5초 상한으로 drain한다. Readiness는 full resync의 `mesh_resync_pending|mesh_resync_failed` blocker로 노출한다.
