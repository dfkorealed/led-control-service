...
- Node별 capability journal은 model binding 결과가 실제로 달라질 때만 revision과 event ID를 새로 만든다. Commit uncertainty는 target이 previous면 이전 identity 유지, next면 같은 revision/eventId/hash 채택, unknown이면 journal을 fence한다. Broker PUBACK 뒤에도 report를 유지하고 exact application ACK만 terminal로 만든다. Provisioning completed terminal은 QoS1 callback을 먼저 보장하고 capability refresh는 별도 durable pending node queue와 health blocker에서 재시도한다.
