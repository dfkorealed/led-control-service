...
- Gateway는 automation state schema v3에 실제 관측 또는 성공 terminal로 확인된 desired와 restart observation이 필요한 legacy desired를 분리한다. V1 및 terminal 증거 없는 V2 desired와 V2/V3 pending은 lighting observation까지 RF를 보류하고, target과 관측 brightness가 같으면 terminal로 확정하며 다를 때만 RF를 전송한다. RF success 뒤 terminal state write failure/commit uncertainty도 same-process fence를 즉시 세운다. 이 fence 집합은 durable pending transition에서 재시작마다 복원되며 `pendingObservationFixtureIds()` snapshot이 recovery worker의 source-of-truth다. Targeted queue는 4,096개 capacity와 64개 batch로 요청을 부분 수용하고 overflow 시작점을 보존한다. 각 targeted/full pass 뒤 아직 실제 관측으로 해제되지 않은 ID를 source에서 다시 읽어 회전된 capacity window로 교체하므로 영구 offline fixture의 fence를 유지하면서 뒤 fixture도 관측 기회를 얻는다. Durable OnOff/Lightness 관측 반영 뒤에만 fence와 queue 항목을 해제한다.
