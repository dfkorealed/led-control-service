...
- Vendor event는 version, `bootId`, `sequence`, `eventKind`, `level`의 exact length와 값 조합을 검증한다. `(sourceUnicast,bootId,sequence)` dedupe를 원자 파일과 manifest에 저장하고 runtime state가 durable해진 뒤 dedupe를 저장한 다음 ACK한다. Duplicate와 이전 sequence에도 ACK를 다시 보내며 bootId 변경은 새 session이다. 알 수 없거나 미설정된 source, unknown property와 malformed payload는 상태를 바꾸지 않고 주소와 정제 reason만 기록한다.
- Node별 capability journal은 model binding/publication 결과가 실제로 달라질 때만 revision과 event ID를 새로 만든다. Complete report와 canonical hash는 publish 전에 원자 저장하고 broker PUBACK 뒤에도 유지한다. 1~30초 bounded retry와 reconnect는 동일 report를 재발행하며 exact application ACK의 event/gateway/node/revision/hash만 terminal로 만든다. Rejected와 hash mismatch는 journal을 보존한 fail-closed 진단이며 production MQTT ACL은 capability ACK를 Gateway read-only로 제한한다.
