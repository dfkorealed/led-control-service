...
- Gateway는 활성 occurrence, timed manual override, 차량 source/hold, source 시작 전 brightness, 실제 관측 current, fixture별 `pending|terminal` transition과 성공한 마지막 desired를 `/var/lib/led-control/automation-state.json`에 Task 11과 같은 temp write/fsync/rename/parent fsync 방식으로 원자 저장한다. 성공 terminal commit만 동일 desired를 억제하며 pre-send/terminal 실패와 RF 뒤 commit uncertainty는 restart에서 at-least-once로 재시도한다.
- Schedule recurrence는 wall clock, 현재 process의 vehicle Low hold와 timed manual expiry는 monotonic clock을 사용한다. Restart expiry만 trusted UTC를 새 monotonic deadline으로 변환한다. Production clock trust는 Compose가 read-only mount한 `/run/systemd/timesync` 아래 marker와 5분 이상 wall-clock rollback을 검사하고 rollback 순간 marker mtime 이후 갱신만 신뢰 회복으로 인정한다. Untrusted 동안 새 schedule boundary와 restart UTC expiry 전이만 보류하고 MQTT/manual 입력, sensor current state와 현재 process monotonic hold는 계속 처리한다.
- Timed manual command는 override pre-state와 pending transition을 RF보다 먼저 durable 저장한다. Command journal의 `automationHandoff` phase가 prepare-before-RF crash를 indeterminate terminal로 정리하고 completed-before-handoff crash를 RF 없이 재생한다. 성공한 manual-only override 만료는 마지막 수동 밝기를 유지하며 활성 event/schedule이 있으면 우선순위대로 복귀한다.
