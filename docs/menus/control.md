...
- Task 16 model worker와 static queue는 최초 start에서 한 번만 생성하고 stop 때 generation별 parked ack를 공개한 뒤 같은 task를 재사용한다. Sensor/vendor send fault active 조건은 분리되며 Sensor 성공은 vendor 장애를 지우지 않고 exact event ACK 또는 vendor 성공만 vendor fault를 회복한다. Registered history Clear와 permanent sequence exhaustion current 의미는 유지한다.
- Firmware production trust anchor/fingerprint는 caller env가 아닌 고정 repository/CI policy에서만 읽으며 현재 `unprovisioned`라 production build는 의도적으로 실패한다. 향후 v2 approval은 CID/source commit/sdkconfig/partition digest를 서명하고 production artifact attestation은 approval identity와 app/bootloader/partition-table/otadata hash를 release key로 서명한다. Flash wrapper는 exact signature/hash를 재검증한다. Test `0xFFFF` image는 app 첫 분기에서 fail-stop해 raw flash 우회에도 NVS/Bluetooth/sensor를 시작하지 않는다.
...
- Task 14 Gateway Sensor/vendor client, Task 15 GPIO driver와 Task 16 ESP32-H2 model은 native exact wire/retry, actual production-source host fake와 ESP-IDF fullclean target build로 검증했다. Model host fake는 config queue 포화와 reboot 복원, period/AppKey/reprovision, 네 Sensor 요청, authoritative current unavailable, 단일 timer, stop race/restart와 Health recovery를 실행한다. 실제 RF, cache-disabled ISR, 센서 전기 신호, packet loss, 전원 차단은 증명하지 않으며 HIL은 아직 실행하지 않았다.
