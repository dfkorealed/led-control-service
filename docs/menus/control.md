...
- Gateway는 automation state schema v3에 실제 관측 또는 성공 terminal로 확인된 desired와 restart observation이 필요한 legacy desired를 분리한다. V1 및 terminal 증거 없는 V2 desired와 V2/V3 pending은 lighting observation까지 RF를 보류하고, target과 관측 brightness가 같으면 terminal로 확정하며 다를 때만 RF를 전송한다. RF success 뒤 terminal state write failure/commit uncertainty도 same-process fence를 즉시 세우고 해당 fixture를 bounded targeted lighting-resync queue에 요청한다. 요청 실패나 미관측은 지수 backoff로 재시도하고 unresolved batch를 insertion-order tail로 회전시켜 선두 64개가 offline이어도 이후 fixture가 관측 기회를 얻는다. Durable OnOff/Lightness 관측 반영 뒤에만 fence와 queue 항목을 해제한다.
- Automation fixture observation은 Health Current telemetry와 분리된 OnOff/Lightness callback으로 수집한다. Health timeout이어도 밝기 recovery는 완료된다. Full resync는 MQTT/heartbeat/ACK/manual control startup 뒤 bounded-concurrency background worker로 실행하고 fixture 실패를 격리한다. Full/targeted worker는 fixture 경계, 관측 대기, busy retry에 `AbortSignal`을 전달하고 production shutdown은 기본 5초 상한으로 drain한다. Readiness는 full resync의 `mesh_resync_pending|mesh_resync_failed` blocker로 노출한다.
- Schedule recurrence는 wall clock, 현재 process의 vehicle Low hold와 timed manual expiry는 monotonic clock을 사용한다. 새 command delivery는 durable `deliveryGeneration`, `deliveryGeneratedAt`, `deliveryWindowMs`, `overrideRemainingMs`와 broker packet의 remaining TTL로 transit age를 차감해 1시간/30일 override를 10초로 자르지 않는다. Command receipt에서 wall trust와 무관한 monotonic execution deadline을 만들고 journal/acceptance/automation persistence 뒤와 RF 직전에 다시 검사한다. Untrusted restart에서는 absolute expiry를 아직 판정할 수 없는 recovered manual의 현재 출력을 manual 최고 우선순위로 유지해 active event/schedule takeover와 duplicate RF를 막고, clock trust 회복 뒤에만 absolute remaining을 현재 boot의 monotonic deadline으로 변환한다. Metadata가 없는 old-API timed wire도 인증된 broker packet의 remaining TTL이 10초 freshness 경계 안에 있을 때 compatibility version 1로만 허용한다. 이 경로는 `overrideUntil - requestedAt` 요청 duration에서 packet transit/수신 후 경과를 빼 현재 boot의 monotonic deadline으로 저장하며 30일을 상한으로 한다. 사용 시 `legacy_timed_manual_wire_compatibility` 구조화 경고를 남긴다.
