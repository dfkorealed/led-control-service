...
- Gateway는 automation state schema v4에 실제 관측 또는 성공 terminal로 확인된 desired와 restart observation이 필요한 legacy desired를 분리한다. V1~V3은 v4로 migration하며 lifecycle/terminal state transition과 stable `pendingTelemetryHandoffs { handoffId, recordsHash, exact records }`를 같은 atomic mutation에 저장한다. Outbox의 idempotent batch acceptance 뒤 exact state handoff를 clear해 모든 cross-file crash 경계를 재생한다. Restart pending/unverified desired는 lighting observation까지 RF를 보류하고 target과 다를 때만 RF를 전송한다. RF success 뒤 terminal state write failure/commit uncertainty도 same-process fence를 즉시 세우며 process-local monotonic deadline은 commit 성공 뒤에만 교체된다.
...
- Gateway execution telemetry `appendBatch`는 handoff 전체를 한 atomic rewrite로 수락하거나 실제 dropped record set 전부를 반환해 prefix success를 만들지 않는다. Grouped `action_result` 한 payload의 여러 fixture result는 dropped count 1이다. Pretty JSON metadata를 포함한 regular outbox는 strict 64 MiB다. Automation state와 outbox는 startup에 한 번 실제 할당한 공용 64 MiB headroom을 사용하며 정상 commit은 reserve I/O를 만들지 않는다. 실제 `ENOSPC`에서만 reserve를 한 번 release/retry하고 verified free space 뒤 background로 복원한다. State mutation은 `durable|memory_only` outcome을 반환하고 schedule/vehicle/manual local transition만 memory-only fallback을 사용한다. Handoff/gap clear는 durable-required라 실패 시 memory/disk pending과 outbox receipt를 그대로 두고 coordinator batch를 중단해 1초~30초 bounded backoff로 같은 handoff를 재시도한다. Fixed journal은 마지막 일반 source와 별도로 cumulative gap source receipt 한 개만 유지해 메모리·파일 크기를 고정하고, 다른 handoff가 사이에 기록돼도 오래된 state count를 다시 더하지 않는다. Durable clear 뒤에만 receipt를 release하므로 restart도 동일 event ID/sequence/hash로 수렴하고 gap count를 늘리지 않는다. Retry에서 처음 outbox work가 생기면 publisher를 깨우며, 미게시 `event_extended`는 latest record로 교체하고 MQTT application ACK exact match 전 record를 유지한다.
