...
- Gateway는 automation state schema v3에 실제 관측 또는 성공 terminal로 확인된 desired와 restart observation이 필요한 legacy desired를 분리한다. V1 및 terminal 증거 없는 V2 desired와 V2/V3 pending은 startup local Mesh resync까지 RF를 보류하고, target과 관측 brightness가 같으면 terminal로 확정하며 다를 때만 RF를 전송한다. Automation fixture observation은 telemetry reservation과 독립된 listener로 수집한다.
- Schedule recurrence는 wall clock, 현재 process의 vehicle Low hold와 timed manual expiry는 monotonic clock을 사용한다. 현재 process manual은 clock-untrusted 중 수신해도 server-confirmed `startedAt -> overrideUntil` duration으로 deadline을 만든다. Restart expiry만 trusted UTC를 새 monotonic deadline으로 변환한다. Production clock trust는 Compose가 read-only mount한 `/run/systemd/timesync` 아래 marker와 rollback 이후 marker 갱신을 검사한다.
