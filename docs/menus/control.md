...
- capability report/ACK shared 계약, node-local ordered/idempotent 원자 적용 service와 durable ACK outbox 생성은 완료했지만 MQTT subscription/consumer와 ACK outbox publisher는 아직 없다. Task 9는 mTLS/broker ACL identity, topic/payload scope, active claimed Gateway를 모두 검증한 뒤 service를 호출하고 service가 저장한 ACK variant를 publish해야 하며 unauthenticated direct route를 만들지 않는다.
