...
- Task 16 model worker와 static queue는 최초 start에서 한 번만 생성하고 stop 때 generation별 parked ack를 공개한 뒤 같은 task를 재사용한다. ESP-IDF model-only completion은 개별 publish에 상관할 수 없어 advisory no-op로 처리하며 completion ledger와 same-model in-flight 차단은 없다. Sensor는 동기 API 수락/거부, vendor는 동기 API 결과와 exact ACK/retry exhaustion만 Health와 liveness에 반영한다. Sensor/vendor send fault active 조건은 분리되며 restart는 내부 zero Health source를 외부 current/registered 배열에도 exact 반영한다. Registered history Clear와 permanent sequence exhaustion current 의미는 유지한다.
