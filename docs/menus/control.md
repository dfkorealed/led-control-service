...
- Gateway 재시작 시 정상적인 durable group state는 그대로 복원한다. state 파일이 없거나 state/manifest revision이 다르거나 손상됐을 때는 같은 `eventId`의 전체 resync 요청을 영속화하고, API가 DB 반영 후 보낸 애플리케이션 ACK의 `requestEventId`를 확인할 때까지 재시작·재연결·heartbeat에서도 재전송한다. 해당 기간 group 제어는 fail-closed하며, group state 저장 실패 중에도 마지막으로 검증된 전체 snapshot을 보존한다. API는 신규 resync 이벤트마다 해당 Gateway의 각 cloud group `configurationVersion`을 정확히 한 번 증가시켜 `configuring`으로 전환하고, 동일 `eventId` 재전달에는 버전을 다시 올리지 않는다. 이전 버전의 지연 subscription 결과는 무시된다.
