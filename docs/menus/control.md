...
- 이벤트 dialog는 감지 센서, 제어 조명, 행동·유지시간 순서로 구성한다. source picker는 Dashboard fixture의 Gateway 등록, `supported` vehicle sensor capability 및 canonical ISO verified timestamp가 모두 확인된 fixture만 노출한다. timestamp 누락·`null`·비정상 값은 fail-closed한다. target은 direct fixture selection만 지원하며 API의 단일 Gateway 제약을 그대로 적용한다. submit validation 오류는 stable error id로 해당 input/control group과 연결하고 첫 invalid group 또는 input으로 focus를 이동한다.
