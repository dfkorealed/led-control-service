...
- Gateway는 automation state schema v3에 실제 관측 또는 성공 terminal로 확인된 desired와 restart observation이 필요한 legacy desired를 분리한다. V1 및 terminal 증거 없는 V2 desired와 V2/V3 pending은 lighting observation까지 RF를 보류하고, target과 관측 brightness가 같으면 terminal로 확정하며 다를 때만 RF를 전송한다. RF success 뒤 terminal state write failure/commit uncertainty도 same-process fence를 즉시 세운다.
- Automation fixture observation은 Health Current telemetry와 분리된 OnOff/Lightness callback으로 수집한다. Health timeout이어도 밝기 recovery는 완료된다. Full resync는 MQTT/heartbeat/ACK/manual control startup 뒤 bounded-concurrency background worker로 실행하고 fixture 실패를 격리한다. Readiness는 `mesh_resync_pending|mesh_resync_failed` blocker로 노출하며 shutdown은 in-flight resync를 drain한다.
- Schedule recurrence는 wall clock, 현재 process의 vehicle Low hold와 timed manual expiry는 monotonic clock을 사용한다. Trusted manual은 absolute `overrideUntil` remaining만 사용한다. Clock-untrusted 수신은 absolute command-expiry 판정을 건너뛰고 broker retention 상한과 같은 최대 10초 fail-safe monotonic window로 제한해 원래 최대 30일 duration을 수신 시점부터 다시 시작하지 않는다. Restart expiry만 trusted UTC를 새 monotonic deadline으로 변환한다.
...
- `POST /commands/dimming`은 optional ISO instant `overrideUntil`을 받고, 없으면 API `AutomationClock` 기준 `now + 1 hour`를 서버에서 확정한다. 명시 시각은 현재보다 미래이고 최대 30일 이내여야 하며 viewer는 기존과 같이 `403`으로 거부된다. Command, `ManualOverride`, 모든 `ManualOverrideFixture`, dispatch와 MQTT outbox는 공통 automation advisory lock 후 Site row 재인가를 거친 하나의 transaction에 저장한다. API publisher는 absolute override가 지난 row를 발행/재발행하지 않고 `MANUAL_OVERRIDE_EXPIRED`로 종료하며 command/MQTT expiry도 override end를 넘지 않는다. 이전 durable outbox payload는 필드 없이도 처리한다.
