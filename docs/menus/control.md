...
- capability report/ACK shared 계약, node-local ordered/idempotent 원자 적용 service와 durable ACK outbox 생성은 완료했다. ACK identity는 `vehicle-sensor-capability:<gatewayId>:<meshNodeId>:<eventId>`이며 같은 Gateway의 cross-node eventId 충돌도 원본을 덮지 않는 별도 rejected ACK가 된다. Exact report 재전달은 최초 payload/hash/`ingestedAt`을 유지하고 published/deadletter/expired lease row만 즉시 재큐잉하며 live lease는 보호한다. MQTT subscription/consumer와 publisher는 아직 없다. Task 9는 mTLS/broker ACL identity, topic/payload scope, active claimed Gateway를 검증한 뒤 service를 호출하고, config와 application-ACK를 variant별 `FOR UPDATE SKIP LOCKED` 30초 lease로 claim해 저장 payload를 QoS 1으로 발행해야 한다. 실패는 1초~60초 backoff 후 10회 또는 15분에 retained deadletter로 남기고, single-flight `stopAndDrain()` 뒤 MQTT를 닫는다. Unauthenticated direct route는 만들지 않는다.
