...
- Task 20 Fix Round 2에서 shared output lock은 token, PID와 `ps` process-start identity를 `.owner.<token>` strict marker로 기록한다. unique temp directory에 marker를 완전히 쓴 뒤 `rename(temp, lock)`으로 publish하므로 보이는 non-empty lock은 항상 완성 owner다. active owner는 wait, missing PID와 start identity가 다른 PID reuse는 exact old marker unlink 뒤 빈 lock directory `rmdir`과 재시도로 복구하며, identity 미확인은 fail-closed다. release도 exact marker만 unlink하고 successor marker의 `ENOENT`/non-empty directory는 건드리지 않는다. empty lock은 release crash artifact로만 `rmdir`/retry하고 temp symlink·non-directory·외부 sentinel·orphan temp는 strict marker/token 검사로 보호한다. quarantine rename과 mtime steal은 사용하지 않는다. production `scripts/esp32-h2-build.sh`는 Bluetooth SIG 자사 Company ID와 signed manufacturing approval이 없는 현재 `unprovisioned` policy에서 의도적으로 fail-closed한다. 이는 HIL 실패나 HIL 완료 증거가 아니다.
