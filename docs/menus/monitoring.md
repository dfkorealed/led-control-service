...
- 조명이 1개 이상인 일반 모니터링 화면에서는 admin에게 `조명 등록` 버튼을 제공한다. 버튼으로 연 dialog가 기존 등록 패널을 mount하므로 provisioning 완료로 dashboard 조명 수가 증가한 뒤에도 active 세션을 복구해 완료·취소하거나 추가 조명을 검색할 수 있다. 조명 0개 초기 상태에서는 등록 패널을 화면에 직접 표시한다.
