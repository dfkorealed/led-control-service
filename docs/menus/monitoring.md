...
- MQTT certificate rotation은 manual-connect candidate 전에 기존 stable client를 quiesce해 reconnect takeover 경합을 막고, candidate가 받은 QoS 1 command는 journal-backed handler로 즉시 처리해 rollback payload replay를 하지 않는다. 정상 rollback만 기존 client를 명시적으로 reconnect하며, pointer write/fsync와 restore가 모두 실패하면 candidate generation을 보존하고 runtime을 fail-closed 한다. appliance health는 D-Bus owner, 실제 `Node1` interface introspection, HCI powered bit, mapping JSON parse, 마지막 heartbeat publish freshness를 실제 probe해 기록하며 future heartbeat와 잘못된 heartbeat interval은 unhealthy로 처리한다.
