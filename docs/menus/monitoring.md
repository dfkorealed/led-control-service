...
- API provisioning scan/command outbox worker는 initial·interval batch의 transient DB 실패를 scheduler 경계에서 격리하고 worker별 single-flight로 실행한다. 종료가 시작되면 다음 record publish를 시작하지 않으며 `MqttShutdownCoordinator`가 두 worker의 멱등 `stopAndDrain()`을 모두 완료한 뒤에만 `MqttService.close()`를 호출한다. MQTT close는 graceful `end` callback을 await하고 5초 timeout에 force close callback을 추가 1초간 기다려 영구 hang 없이 Nest module 종료를 마친다. worker와 `MqttService`에는 coordinator와 경쟁하는 독립 destroy hook이 없다.
...
- 등록 패널은 `pending/scanning` 또는 provisioning 중에만 1.5초 registration session polling으로 결과를 반영한다. 실시간 push와 단계별 진행률은 명시적 보류 범위이며, `reconcile_required` 장비의 현장 확인·복구 workflow는 후속 구현이 필요하다.
