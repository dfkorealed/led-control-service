...
- `automation_state_durability_degraded`: shared headroom release/retry 뒤에도 state atomic write가 `ENOSPC`여서 Gateway가 in-memory committed state로 제어를 계속하고 있다. Schedule, vehicle event와 manual RF가 계속되는 것이 정상이며 새 exact handoff는 preallocated `.gap` journal로 전환된다. 같은 filesystem의 비-Gateway 로그/임시 파일을 정리하고 반복 재시작하지 않는다. Free space가 회복되면 다음 state mutation이 전체 in-memory snapshot을 저장하고 blocker를 해제하며, process가 먼저 종료됐다면 durable 이전 state와 `telemetry_gap`, startup Mesh observation으로 현장 상태를 reconcile한다.
