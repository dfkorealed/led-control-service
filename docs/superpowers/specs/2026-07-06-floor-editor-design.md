...
상태: 구현 전 설계. 아래 내용은 최신 사용자 요청을 반영하며, 하단의 기존 MVP 기록과 충돌하면 이 절을 우선한다. 실행 체크리스트는 `../plans/2026-07-06-floor-editor-implementation.md`의 `2026-09-09 대량 배치 실행 계획`이다. 기존 구현을 완료로 재분류하지 않는다.
...
- 등록 여부와 지도 배치는 별개다. 제안 데이터는 `Fixture.placementStatus: unplaced | placed`, `Fixture.positionVerifiedAt: DateTime?`이며, 공유 계약과 migration은 실행 Task 1에서 소유 역할 검토 후 적용한다.
