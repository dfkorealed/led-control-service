...
| 스케줄·차량 이벤트 제어 Task 13 | 완료(소프트웨어, fix round 5) | 최초 gap의 stable identity와 full-disk cumulative retry를 유지하면서 fixed journal aggregate에 outbox가 durable 수용한 누적 baseline과 acceptance handoff/hash를 in-place 보존한다. Reimport는 교체되는 last/cumulative source metadata가 아니라 `aggregate - acceptedBaseline`만 처리하고, source receipt는 state replay identity로만 사용한다. Outbox commit 뒤 baseline/clear의 failure 또는 previous·next visible uncertainty, general source 교체와 restart에서도 동일 `telemetry_gap` event identity/sequence/final hash와 정확한 count로 수렴한다. 두 4 KiB block, 64 MiB outbox/headroom, local RF 독립성과 application ACK 계약은 유지한다. Gateway 494/494, shared 74/74, Docker 17/17, Mosquitto 2/2를 통과했으며 실제 Raspberry Pi power-loss/storage-wear와 ESP32-H2 RF HIL은 별도다. |
