...
| 스케줄·차량 이벤트 제어 Task 16 | 완료(소프트웨어/target build, Fix Round 2) | Fix Round 1 잔여 P2 2건을 TDD로 수정했다. Static model worker는 한 번만 생성해 generation별 park/restart하고, Sensor/vendor send fault active 조건과 exact ACK recovery를 채널별로 분리했다. 100회 즉시 stop/start에서 task create 1회/delete 0회와 stale command 부재를 검증했다. Gateway 546/546, shared 75/75와 ESP-IDF clean test-build를 통과했으며 binary `0xef980`, OTA free `0x100680`이다. Production trust는 정상 fail-closed이고 실제 flash/RF/HIL은 미실행이다. |
