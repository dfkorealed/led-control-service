...
| 스케줄·차량 이벤트 제어 Task 12 | 완료(소프트웨어, fix round 1) | Task 11 activation seam에 common recurrence scheduler와 `manual > max vehicle > schedule > current/default` arbiter를 연결했다. Fix round 1은 RF와 telemetry capacity를 분리한 durable gap seam, pending→terminal 성공-only dedup과 at-least-once crash retry, manual-only 최종 밝기 유지, timesync directory mount/rollback fence, manual journal handoff replay, monotonic manual expiry와 async shutdown drain을 보강했다. Focused 100/100, Gateway 373/373, Docker 17/17, Mosquitto 2/2와 typecheck/lint/build를 통과했으며 Raspberry Pi/ESP32-H2 RF HIL은 미실행이다. |
