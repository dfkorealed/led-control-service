...
| 스케줄·차량 이벤트 제어 Task 13 | 완료(소프트웨어, fix round 2) | State와 strict 64 MiB telemetry outbox가 startup 1회 preallocated 64 MiB headroom을 공유한다. 정상 commit은 reserve I/O가 없고 실제 `ENOSPC`만 one-shot release/retry하며 verified free space 뒤 background replenish한다. State retry 실패는 exact 새 handoff를 8 KiB in-place gap journal로 전환하고 in-memory committed state로 schedule/vehicle/manual RF를 계속하며, 이후 full snapshot을 durable state로 reconcile한다. Typed commit uncertainty, visible-target/restart fence와 idempotent coordinator가 records/gap 이중 분류를 막는다. MQTT QoS 1 immutable retry, exact application ACK 삭제/conflict 보존과 shutdown drain은 유지한다. 실제 Raspberry Pi power-loss/storage-wear와 ESP32-H2 RF HIL은 별도 검증이다. |
