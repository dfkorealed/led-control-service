...
| 스케줄·차량 이벤트 제어 Task 18 | 완료(소프트웨어/Web unit) | event rule API CRUD를 Web에 연결해 admin 생성·수정·삭제·활성화, viewer read-only 목록, source·target·brightness·hold·Gateway sync·최근 감지를 제공한다. source picker는 Dashboard의 Gateway 등록과 `supported`·verified vehicle capability를 모두 확인한 fixture만 노출한다. 수동 command에는 local 종료 시각 기본 1시간, 빈 값의 서버 기본값 유지, ISO `overrideUntil` 변환과 과거·30일 초과 client 거부를 추가했다. focused Web 63/63, Web 전체 342/342, Web production build와 Dashboard capability API unit 5/5·typecheck를 통과했으며 production API/Gateway Chromium E2E와 Raspberry Pi/ESP32-H2 HIL은 Task 19 이후 별도다. |
...
- 스케줄과 차량 이벤트 규칙은 Web admin CRUD·활성화와 viewer read-only, Gateway 동기화·pagination/polling 복구·세션 만료까지 연결했다. 차량 이벤트 source는 Gateway 등록과 supported·verified sensor capability가 확인된 Dashboard fixture로 제한하고, 수동 override 종료 시각은 client에서 과거·30일 초과를 거부한다. production API/Gateway Chromium E2E는 Task 19 범위로 남아 있다.
