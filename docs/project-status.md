...
| 스케줄·차량 이벤트 제어 Task 8 | 완료(소프트웨어) | per-node capability revision, canonical hash/event ledger dedupe, stale/out-of-order 처리, strict ACK와 Task 9/14 인증·durable retry handoff까지 3차 review를 완료했다. production MQTT consumer와 Gateway report journal은 후속 Task 범위다. |
...
- 차량 센서 capability report/ACK payload와 topic, API ordering/idempotency service 및 DB invariant는 완료했다. production MQTT consumer, mTLS/ACL identity 검증과 durable ACK publication은 Task 9, Gateway Sensor Server/vendor model 검증과 durable revision/report retry는 Task 14까지 연결되지 않는다.
