...
| 스케줄·차량 이벤트 제어 Task 13 | 완료(소프트웨어, fix round 1) | State schema v4가 lifecycle/terminal transition과 stable exact telemetry handoff를 함께 저장하고 outbox가 batch를 원자·멱등 수락한 뒤 state를 clear한다. Strict 64 MiB regular outbox는 별도 preallocated rewrite reserve를 사용하며 포화/`ENOSPC` drop은 8 KiB fixed two-block gap journal의 in-place write로 격리한다. Corrupt/`ENOSPC` telemetry cold start에서도 scheduler/local RF를 계속하고, process-local hold deadline은 state commit 뒤에만 변경하며 persisted gap replay count를 중복하지 않는다. MQTT QoS 1 immutable retry, exact application ACK 삭제/conflict 보존과 shutdown drain은 유지한다. 실제 ESP32-H2 sensor model과 RF/storage-wear HIL은 Task 14 이후 별도 검증이다. |
