...
| 스케줄·차량 이벤트 제어 Task 16 | 완료(소프트웨어/target build, Fix Round 1) | 독립 리뷰 P1 2건/P2 5건을 TDD로 수정했다. Mesh settings 영속화, queue 독립 config latch, Sensor stack period 0과 custom 단일 cadence, Descriptor/Get/Column/Series 공식 응답, Task 15 authoritative current, producer-drain shutdown, active/history Health와 4개 책임 모듈 분리를 production-source host fake로 검증했다. Gateway 546/546, shared 75/75와 ESP-IDF clean test-build를 통과했으며 binary `0xef710`, OTA free `0x1008f0`이다. Production trust는 정상 fail-closed이고 실제 flash/RF/HIL은 미실행이다. |
