...
| 스케줄·차량 이벤트 제어 Task 12 | 완료(소프트웨어, fix round 3) | Task 11 activation seam에 common recurrence scheduler와 `manual > max vehicle > schedule > current/default` arbiter를 연결했다. Fix round 3는 Health와 독립된 OnOff/Lightness observation, RF terminal commit failure의 same-process fence, MQTT-first bounded background Mesh resync와 readiness/drain, Cloud absolute override expiry 및 clock-untrusted 10초 fail-safe를 보강했다. Shared focused 30/30, API focused 49/49, Gateway focused 160/160, Gateway full 400/400, Docker 17/17, Mosquitto 2/2와 shared/API/Gateway typecheck/lint/build를 통과했으며 Raspberry Pi/ESP32-H2 RF HIL은 미실행이다. |
