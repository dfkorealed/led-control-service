...
| 스케줄·차량 이벤트 제어 Task 16 | 완료(소프트웨어/target build, Fix Round 3) | Fix Round 2 재검토 P2 2건을 TDD로 수정했다. ESP-IDF callback에 context/opcode가 없는 제약은 Sensor/vendor별 2-slot fixed ledger와 model 단위 단일 in-flight로 해결해 이전 generation completion을 무시한다. Restart는 내부 zero active/history를 외부 Health current/registered 배열까지 exact 재구성한다. 기존 100회 lifecycle, Gateway 546/546, shared 75/75와 ESP-IDF fullclean test-build를 통과했으며 binary `0xefc70`, OTA free `0x100390`이다. Production trust는 정상 fail-closed이고 실제 flash/RF/HIL은 미실행이다. |
