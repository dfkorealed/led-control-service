...
| 스케줄·차량 이벤트 제어 Task 16 | 완료(소프트웨어/target build, Fix Round 4) | ESP-IDF v5.5.1 model-only completion을 개별 송신 상태에서 제거해 callback 유실·지연·중복과 same-model 재사용이 liveness/Health를 오염할 경로를 없앴다. Sensor는 동기 API 결과, vendor는 동기 API 결과와 exact ACK/retry exhaustion만 신뢰하며 event별 initial+6 retry 실제 publish를 보장한다. Restart zero Health, 100회 lifecycle, Gateway 546/546, shared 75/75와 ESP-IDF fullclean test-build를 통과했고 binary `0xef920`, OTA free `0x1006e0`이다. Production trust는 정상 fail-closed이고 실제 flash/RF/HIL은 미실행이다. |
