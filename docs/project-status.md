...
| 스케줄·차량 이벤트 제어 Task 15 | 완료(소프트웨어/target build, fix round 1) | Review P1 4/P2 4를 TDD로 수정했다. IRAM config/map audit, ordered boot reconciliation, overflow authoritative resync, UART0 pin exclusion과 callback self-stop 거부를 actual-driver host fake로 검증했다. Test image는 부팅 첫 분기에서 side-effect 전 fail-stop하고, production은 exact CID signed manufacturing approval가 없으면 실패한다. Artifact hash binding, custom `0x1f0000` two-OTA slot과 20%/256 KiB release margin gate를 적용했다. Clean test-build는 `0xe64f0`, free `0x109b10`이며 실제 센서 전기/cache-disabled/RF/HIL은 미실행이다. |
