...
| 스케줄·차량 이벤트 제어 Task 16 | 완료(소프트웨어/target build, Fix Round 5) | Application-driven Sensor Status/vendor event를 ESP-IDF deep-copy server-send로 전환해 BTC backlog에서도 back-to-back sequence 1/2, 16-slot same-deadline retry와 Sensor recovery payload snapshot을 보존한다. NetKey `0`과 publication의 AppKey/address/TTL/credential/SZMIC를 exact 반영하고 period/retransmit `0` 계약을 firmware/Gateway 양쪽에서 fail-closed 검증한다. Completion은 advisory no-op이며 exact ACK/retry exhaustion과 synchronous enqueue 결과가 source of truth다. Gateway 547/547, shared 75/75와 ESP-IDF fullclean test-build를 통과했고 binary `0xef9f0`, OTA free `0x100610`이다. Production trust는 정상 fail-closed이고 실제 flash/RF/HIL은 미실행이다. |
