...
| 스케줄·차량 이벤트 제어 Task 20 | 완료(문서·software 최종 검증, Fix Round 1) | shared build output lock은 token/PID/실제 process-start identity를 strict metadata로 기록해 active owner, missing PID, PID reuse, identity unknown을 분리한다. stale lock은 atomic quarantine takeover로 복구하고 release는 exact ownership match일 때만 수행하며, lock/metadata/quarantine symlink·비정상 파일은 fail-closed한다. focused RED/GREEN은 정상 concurrent build, crash stale recovery, PID reuse, timeout, release fencing, cleanup refusal, external sentinel과 package root 독립성을 포함한다. HIL runbook은 build/flash에 같은 exported Company ID, approval manifest/signature와 `IDF_PATH`를 전달하고, retrigger High/즉시 Low 뒤 old/new deadline의 80%/40% 관측을 분리하며, MQTT capture PID/trap/kill/wait를 기록한다. production `scripts/esp32-h2-build.sh`는 자사 Bluetooth SIG Company ID와 signed manufacturing approval이 없는 현재 `unprovisioned` policy에서 의도적으로 fail-closed했다. 실제 Raspberry Pi/BlueZ/ESP32-H2 HIL은 실행하지 않았다. |
