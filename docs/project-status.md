...
| 스케줄·차량 이벤트 제어 Task 20 | 완료(문서·software 최종 검증, Fix Round 3) | shared build output lock은 complete `.owner.<token>` marker를 temp에서 atomic rename으로 publish하고, fixed lock만 ownership을 판단한다. temp orphan의 empty/partial marker와 complete marker는 exact temp path에서 best-effort 정리하며, temp symlink/non-directory/marker symlink/external sentinel은 건드리지 않고 무시한다. cleanup에 걸린 contender는 fixed lock을 publish하기 전 `ENOENT`/collision으로 retry해 mutual exclusion을 잃지 않는다. active fixed owner, stale/PID reuse, unknown identity, exact marker release, delayed successor ABA, same output 직렬화와 다른 output 독립성을 RED/GREEN으로 검증했다. 정리 불가능한 손상 temp는 남을 수 있지만 fixed lock을 방해하지 않는다. HIL runbook은 build/flash에 같은 exported Company ID, approval manifest/signature와 `IDF_PATH`를 전달하고, retrigger High/즉시 Low 뒤 old/new deadline의 80%/40% 관측을 분리하며, MQTT capture PID/trap/kill/wait를 기록한다. production `scripts/esp32-h2-build.sh`는 Bluetooth SIG 자사 Company ID와 signed manufacturing approval이 없는 현재 `unprovisioned` policy에서 의도적으로 fail-closed했다. 실제 Raspberry Pi/BlueZ/ESP32-H2 HIL은 실행하지 않았다. |
