...
| 스케줄·차량 이벤트 제어 Task 13 | 완료(소프트웨어, fix round 3) | State mutation은 `durable|memory_only` outcome을 반환하고 local schedule/vehicle/manual transition만 memory-only fallback을 허용한다. Telemetry handoff/gap cleanup은 durable-required라 ENOSPC/commit uncertainty에서 memory·disk source와 outbox receipt를 유지하고 coordinator batch를 중단해 1초~30초 bounded backoff로 같은 stable handoff를 재시도한다. Durable clear 뒤에만 receipt를 release하므로 restart가 동일 event ID/sequence/hash 또는 같은 cumulative gap으로 수렴한다. Shared 64 MiB headroom, strict outbox cap, fixed journal, local RF 격리와 MQTT exact ACK/shutdown drain은 유지한다. 실제 Raspberry Pi power-loss/storage-wear와 ESP32-H2 RF HIL은 별도 검증이다. |
