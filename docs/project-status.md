...
| 스케줄·차량 이벤트 제어 Task 20 | 완료(문서·software 최종 검증, Fix Round 2) | shared build output lock은 unique temp directory 안에 strict `.owner.<token>` marker(token/PID/실제 process-start identity)를 완성한 뒤 atomic rename으로 publish한다. stale/release는 읽은 exact marker만 unlink하고 빈 directory만 `rmdir`해 retry하므로 late contender의 old marker `ENOENT`은 successor를 건드리지 않고 abort한다. active owner, missing PID, PID reuse, identity unknown, empty release artifact, temp symlink/non-directory/external sentinel과 orphan temp cleanup을 RED/GREEN으로 검증했으며 quarantine rename과 mtime steal은 사용하지 않는다. HIL runbook은 build/flash에 같은 exported Company ID, approval manifest/signature와 `IDF_PATH`를 전달하고, retrigger High/즉시 Low 뒤 old/new deadline의 80%/40% 관측을 분리하며, MQTT capture PID/trap/kill/wait를 기록한다. production `scripts/esp32-h2-build.sh`는 자사 Bluetooth SIG Company ID와 signed manufacturing approval이 없는 현재 `unprovisioned` policy에서 의도적으로 fail-closed했다. 실제 Raspberry Pi/BlueZ/ESP32-H2 HIL은 실행하지 않았다. |
