...
| 스케줄·차량 이벤트 제어 Task 12 | 완료(소프트웨어, fix round 4) | Fix round 3 re-review의 active 5건을 TDD로 수정했다. Legacy journal/old wire compatibility, transaction-durable exact publish generation, broker TTL receipt-relative monotonic deadline, untrusted restart manual 보류, terminal fence targeted lighting resync/retry, full/targeted abort와 bounded shutdown을 Shared 74/74, API 727 passed, Gateway 411/411, Docker 17/17, Mosquitto 2/2로 검증했다. Raspberry Pi/ESP32-H2 RF HIL은 별도다. |
