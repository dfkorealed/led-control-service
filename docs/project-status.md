...
| 스케줄·차량 이벤트 제어 Task 12 | 완료(소프트웨어, fix round 2) | Task 11 activation seam에 common recurrence scheduler와 `manual > max vehicle > schedule > current/default` arbiter를 연결했다. Fix round 2는 V1/V2/pending restart를 startup observed-state resync로 분류해 mismatch에만 RF를 보내고, active automatic pre-state 보존, untrusted-arrival manual monotonic duration, snapshot-first handoff replay, TTL/eviction 보호 pending journal과 bounded capacity error, atomic process-local guard/deadline을 보강했다. Focused 115/115, Gateway 388/388, Docker 17/17, Mosquitto 2/2와 typecheck/lint/build를 통과했으며 Raspberry Pi/ESP32-H2 RF HIL은 미실행이다. |
