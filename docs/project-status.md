...
| 스케줄·차량 이벤트 제어 Task 15 | 완료(소프트웨어/target build) | ESP32-H2 3.3V Active High GPIO 양 edge와 boot level을 static queue/task로 전달하고 task에서 동일 level만 제거한다. ISR은 level/time/enqueue와 queue-full lock-free atomic counter로 제한하며 software debounce/timing filter는 없다. Safe GPIO allowlist와 PWM/reset/strapping/flash/package/USB 충돌을 fail-closed하고 Task 16 callback/current/drop/start/stop API를 app lifecycle에 연결했다. 실제 Company ID 없는 자동 build는 명시적 non-flashable `0xFFFF` test gate로만 통과하며 production은 실제 자사 ID가 없으면 실패한다. 실제 센서 전기/RF/HIL은 미실행이다. |
