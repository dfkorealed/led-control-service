...
create_identity() {
  ...
  if ! openssl ecparam -name prime256v1 -genkey -noout -out "$temporary/manufacturing-ca.key" ||
    ...
    ! openssl ca -batch -config "$config" -extensions station -in "$temporary/station.csr" -out "$temporary/station.crt" >/dev/null 2>&1; then
    rm -rf "$temporary"
...
main() {
  ...
  if [[ "$COMMAND" == "revoke" ]]; then
    local target generation config temporary
    target="$(readlink "$MANUFACTURING_DIR/station.crt")"
    generation="$MANUFACTURING_DIR/${target%/station.crt}"
    config="$generation/openssl.cnf"
    openssl ca -config "$config" -revoke "$generation/station.crt" >/dev/null 2>&1 || die "station certificate 폐기에 실패했습니다. 이미 폐기된 certificate인지 확인하세요."
