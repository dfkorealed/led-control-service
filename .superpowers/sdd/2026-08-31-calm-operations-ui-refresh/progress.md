...
Task 8: initial validation RED (shared build, Web 379/379, typecheck and build passed; full Chromium `85 passed / 3 skipped / 1 failed` because `mvp1.spec.ts` retained a stale `.metric` assertion; initial docs commit `5aed3ff`).
Task 8: fix round 1/5 (semantic MVP E2E assertions aligned without weakening API/site/auth/statistics/settings meaning; `.metric` changed to the `오늘 전력 사용량` named group and the subsequently exposed stale `운영 설정` copy changed to the `설정 개요` heading; commit `4caa27a`).
Task 8: final software validation GREEN (shared build, Web 379/379, typecheck and production build passed; focused Chromium 1/1 and full Chromium `86 passed / 3 skipped`; build 2,381 modules, main `1,039.89 kB / gzip 317.06 kB`, PDF `532.22 kB / gzip 161.49 kB`; existing 500 kB warning remains non-fatal).
Task 8: complete (software validation and truthful documentation convergence complete through `4caa27a`; in-app Browser control surface was unavailable, so manual visual QA Step 3 remains unchecked and automated Chromium does not replace it; Raspberry Pi/BlueZ/ESP32-H2 HIL was not executed).
Final review fix: settings/dirty RED reproduced `7 failed / 27 passed`: the coarse anchor was intercepted by the floor editor capture guard before React could prevent navigation, fake menu/dialog semantics remained, and parent/overview links duplicated `aria-current` on deeper routes.
Final review fix: commissioning RED reproduced `2 failed / 2 passed` with valid enabled fixtures: pending setup secondary measured 34px and registration submit measured 42px; the enabled 390px/320px search and retry actions already met 44px and were retained as explicit coverage.
Final review fix: GREEN replaces the coarse anchor with a route-neutral button, keeps the desktop overview link, uses labelled navigation/list/link semantics and natural Tab/Shift+Tab/Escape order, preserves dirty cancel/confirm plus role/site/operator boundaries, and scopes 44px commissioning actions to mobile customer settings/monitoring screens.
Final review fix: verification GREEN — settings/dirty Vitest `5 files / 48 tests`, related Chromium `65 passed`, Web `36 files / 383 tests`, full Chromium `90 passed / 3 skipped`, shared build, Web typecheck and production build. Build transformed 2,381 modules; main `1,040.07 kB / gzip 317.05 kB`, PDF `532.22 kB / gzip 161.49 kB`; the existing 500 kB warning remains non-fatal.
Final review fix: software and documentation convergence complete in one final changeset; manual in-app Browser QA and Raspberry Pi/BlueZ/ESP32-H2 HIL remain explicitly unexecuted.
Ruling: the final scoped re-review's remaining commissioning P2 is a non-blocking test-evidence limitation, not an observed runtime defect. The <=760px customer-only CSS enforces 44px on the affected enabled actions, 390px measures the commissioning transitions, 320px measures the same scoped search/retry controls, and the branch-wide helper already proves continuous reachable hit areas; retain the truthful warning that the pending fixture is not a production-reachable API combination and that every commissioning state was not re-exercised at 320px. If wrong, a 320px pending/claim state could still be clipped or occluded despite the shared scoped rule, requiring a later truthful API fixture and exact-helper regression, but no current product failure or desktop/operator leakage was found.
Final review: approved with the above adjudicated residual; manual in-app Browser QA, non-zero safe-area measurement, hardware HIL, and the pre-existing >500 kB chunk warning remain open evidence/performance items.
