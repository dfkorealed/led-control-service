...
- `REDIS_URL`을 필수로 검증하는 Nest `RedisModule`과 단일 lazy ioredis client를 추가했다. Nest 종료 시 생성된 client만 `quit()`한다.
...
- lease API는 대상 floor의 `manage` SiteAccess를 확인한다. normal release는 holder user와 token 모두를 확인하고, operator/admin force release는 `floor_editor.lease_force_released` 감사 기록이 성공한 뒤 감사한 holder token만 삭제한다.
