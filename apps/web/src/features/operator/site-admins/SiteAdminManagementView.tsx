...
export function SiteAdminManagementView() {
  ...
  return (
    <section className="operator-admin-management" aria-labelledby="operator-site-admins-heading">
      ...
      {dialog?.type === "delete" ? <DeleteSiteDialog site={dialog.site} returnFocusElement={returnFocusElement} fallbackFocusElement={createCommandRef.current} onDelete={deleteSiteAdmin} onSuccess={() => complete("현장과 관련 데이터를 영구 삭제했습니다.")} onClose={closeDialog} /> : null}
    </section>
  );
}
      ...
      <td>
        {admin ? (
          <div className="operator-row-actions">
            ...
            <button className="danger-action" type="button" aria-label={`${admin.name} 삭제`} onClick={(event) => onOpen({ type: "delete", site }, event.currentTarget)}><Trash2 size={15} aria-hidden="true" /> 삭제</button>
          </div>
