...
describe("SiteAdminManagementView", () => {
  ...
  it("운영자 목록은 실제 데이터로 네 요약과 상태 표를 표시한다", async () => {
    ...
    expect(screen.getByRole("button", { name: "김관리 삭제" })).toBeVisible();
  ...
  it("requires the site name before permanently deleting the site and its admin", async () => {
    renderView();
    await screen.findByText("customer_admin");
  ...
  it("restores focus to the stable create command after deleting an admin removes its trigger", async () => {
    ...
    await waitFor(() => expect(screen.queryByText("인천 물류센터")).not.toBeInTheDocument());
