...
- 관련 subscription 준비가 끝난 모든 MQTT connect/reconnect에서 `events/automation/current-config-request`에 request ID, Site, Gateway, 요청 시각을 담은 strict request를 발행한다. API가 active claim·MQTT certificate ledger와 topic/payload scope를 transaction에서 확인한 뒤 exact latest desired full snapshot을 revive하므로, 최초 연결·세션 소멸·API 재시작 복구는 retained message나 broker session 수명에 의존하지 않는다. 응답 전 request retry는 1초부터 최대 30초의 bounded backoff와 jitter를 사용한다.
- `applied|rejected` ACK는 exact revision/hash와 UUID delivery identity를 포함한 v2 envelope로 `/var/lib/led-control/automation-config-acks.json`에 먼저 저장한다. MQTT.js의 QoS 1 `handleMessage` backpressure 경계가 hot reload와 ACK outbox fsync 완료까지 inbound broker PUBACK을 보류한다. Outbound ACK의 broker PUBACK은 transport attempt 완료일 뿐 record를 삭제하지 않는다. API가 exact snapshot/ACK를 DB transaction에 durable ingest한 뒤 `acks/automation/config-applied-ingested`로 보낸 Site·Gateway·delivery ID·전체 ACK가 모두 일치할 때만 해당 record를 삭제한다. Wrong/old/altered receipt는 record를 유지하며, receipt가 없으면 1초부터 30초까지 bounded backoff로 reconnect/process restart 뒤에도 같은 envelope를 재발행한다.
