...
- Automation 전용 fixture-status listener를 telemetry capacity와 분리하고 startup local Mesh resync를 실행한다. Manual guard, monotonic deadline과 restart observation fence는 대응 atomic state write가 성공한 뒤에만 변경하므로 terminal write failure/commit uncertainty 중 queued tick이 RF를 중복 실행하지 않는다.
