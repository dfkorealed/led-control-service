...
- 새 output은 단일 fixture unicast 또는 동시성 8의 제한된 parallel unicast를 기존 BLE Mesh executor로 실행한다. Shared headroom retry 뒤에도 automation-state write가 `ENOSPC`이면 mutation을 in-memory committed state로 유지하고 새 exact pending handoff를 preallocated gap journal로 옮긴 뒤 memory queue에서 제거한다. `automation_state_durability_degraded`를 노출하지만 schedule/vehicle/manual local RF는 계속하며, 다음 성공 write가 전체 in-memory snapshot을 durable state로 reconcile한다. Process-local vehicle/manual monotonic deadline은 durable 또는 명시적 in-memory commit 성공 뒤에만 교체한다.
...
- Capacity 또는 state/outbox `ENOSPC` drop은 alternate journal block을 fixed-size positional write와 file `fsync`로 갱신해 추가 allocation 없이 `firstDroppedAt|lastDroppedAt|droppedCount`와 마지막 source handoff identity를 보존한다. Regular outbox/headroom 초기화 실패나 outbox JSON 손상은 `automation_telemetry_unavailable`, state fallback은 `automation_state_durability_degraded`로 격리되며 scheduler, manual intake와 local RF startup은 계속한다. Commit uncertainty는 원래 typed error를 유지하고 visible previous/next target만 reconcile하며 unresolved visibility는 restart까지 fence해 같은 handoff를 records와 gap으로 동시에 분류하지 않는다.
